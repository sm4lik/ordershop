...
export const create = async (req: AuthRequest, res: Response) => {
  try {
    const {
      items, deliveryType, deliveryAddress, guestEmail, guestPhone, guestName,
      paymentMethod, paymentType, notes, deliveryDetails, deliveryInstructions
    } = req.body;
    ...
    let deliveryFee = 0;
    if (deliveryType) {
      const [deliveryTypes]: any[] = await pool.query(
        'SELECT fee, free_from_amount FROM delivery_types WHERE id = ? AND is_active = TRUE',
        [deliveryType]
      );
    ...
    const paymentStatus = payType === 'online' ? 'pending' : 'succeeded';

    // Создание заказа
    const [orderResult]: any = await pool.query(
      `INSERT INTO orders (order_number, user_id, guest_email, guest_phone, guest_name,
         status, total_amount, discount_amount, delivery_fee, final_amount, payment_method, payment_type, payment_status, notes)
       VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?, ?, ?, ?)`,
      [orderNumber, userId, guestEmail, guestPhone, guestName, totalAmount, discountAmount, deliveryFee, finalAmount, paymentMethod || 'cash', payType, paymentStatus, notes || '']
    );
    ...
    for (const item of items) {
      ...
      await pool.query(
        ...
        [orderId, item.productId, item.quantity, price, total]
      );
    }
    ...
    if (deliveryType && deliveryAddress) {
      ...
      await pool.query(
        `INSERT INTO deliveries (order_id, delivery_type_id, address, city, street, building, apartment, floor, entrance, intercom, delivery_instructions)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [orderId, deliveryType, addressText, deliveryAddress.city || '',
...
export const getById = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [deliveries]: any[] = await pool.query(
      `SELECT d.*, dt.name as delivery_type_name
       FROM deliveries d
...
export const getByNumber = async (req: AuthRequest, res: Response) => {
  try {
...
export const getAll = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const sortField = validSorts.includes(sortBy as string) ? sortBy : 'created_at';
    ...
    const [countResult]: any[] = await pool.query(
      `SELECT COUNT(*) as total FROM orders o LEFT JOIN users u ON o.user_id = u.id WHERE 1=1${status ? ' AND o.status = ?' : ''}`,
      status ? [status] : []
    );
    ...
    const ordersWithItems = await Promise.all(orders.map(async (o: any) => {
      // Загружаем items
      const [items]: any[] = await pool.query(
        `SELECT oi.id, oi.product_id as productId, oi.quantity, oi.price, oi.total, p.name
         FROM order_items oi
         LEFT JOIN products p ON oi.product_id = p.id
         WHERE oi.order_id = ?`,
        [o.id]
      ...
      const [deliveries]: any[] = await pool.query(
        `SELECT d.*, dt.name as deliveryTypeName
         FROM deliveries d
         LEFT JOIN delivery_types dt ON d.delivery_type_id = dt.id
         WHERE d.order_id = ?`,
        [o.id]
...
export const updateStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [currentOrders]: any[] = await pool.query(
      'SELECT status, final_amount, payment_method FROM orders WHERE id = ?',
      [id]
    );
        ...
        if (categories.length > 0) {
          ...
          await pool.query(
            `INSERT INTO transactions 
             ...
             VALUES ('income', ?, ?, ?, ?, ?, NOW(), ?)`,
            [categoryId, parseFloat(currentOrder.final_amount), `Оплата заказа`, id, currentOrder.payment_method, userId]
          );
...
export const update = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (items && Array.isArray(items)) {
      totalAmount = items.reduce((sum: number, item: any) => sum + item.total, 0);
    }
    ...
    await pool.query(
      `UPDATE orders SET 
       ...
       WHERE id = ?`,
      [guestName, guestPhone, guestEmail, status, paymentMethod, notes, totalAmount, deliveryFee, finalAmount, id]
    );
    ...
    if (deliveryType && deliveryAddress) {
      const [existingDelivery]: any[] = await pool.query(
      ...
      if (existingDelivery.length > 0) {
        await pool.query(
          `UPDATE deliveries SET 
           ...
           WHERE order_id = ?`,
          [deliveryType, deliveryAddress.fullAddress, deliveryAddress.city, deliveryAddress.street,
        ...
        await pool.query(
          `INSERT INTO deliveries (order_id, delivery_type_id, address, city, street, building, 
             apartment, floor, entrance, intercom, delivery_instructions)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [id, deliveryType, deliveryAddress.fullAddress, deliveryAddress.city, deliveryAddress.street,
           deliveryAddress.building, deliveryAddress.apartment, deliveryAddress.floor,
