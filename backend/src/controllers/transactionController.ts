...
export const getAll = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const sortField = validSorts.includes(sortBy as string) ? sortBy : 'transaction_date';
...
export const create = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [categories]: any[] = await pool.query(
      'SELECT id, type FROM transaction_categories WHERE id = ?',
      [category_id]
    );
    ...
    const [result]: any = await pool.query(
      `INSERT INTO transactions 
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [type, category_id, parseFloat(amount), description || '', order_id || null, payment_method || null, transactionDate, userId]
    );
...
export const update = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [existing]: any[] = await pool.query('SELECT * FROM transactions WHERE id = ?', [id]);
